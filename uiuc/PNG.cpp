...
namespace uiuc{
    ...
    HSLAPixel & PNG::getPixel(unsigned int x, unsigned int y) const{
        if(width_==0 || height_==0){
            cerr<<"Error";
        /*Assert function is used to check the validity of certain conditions
        during program execution. If the condition specified in the assert statement
        evaluates to false, it triggers an assertion failure, which typically results 
        in the program terminating and an error message being displayed.*/
            assert(width_>0);
            assert(height_>0);
        }

        if(x>=width_){
            cerr<<"Warning: "<<x<<" is outside the image"<<endl;
            /*The reason for truncating x to (width_ - 1) is to ensure that the coordinate 
            falls within the valid range of the image.*/
            /*In computer graphics, pixel coordinates are typically zero-indexed, meaning
            that the first pixel in an image has coordinates (0, 0). The last pixel in a row 
            has the maximum x-coordinate value of width_ - 1, considering that width_ represents 
            the total number of pixels in a row.*/
            x=width_ - 1;
        }

        if(y>=height_){
            cerr<<"Warning: "<<y<<" is outside the image"<<endl;
            y=height_ - 1;
        }
    ...
    void PNG::resize(unsigned int newWidth, unsigned int newHeight){
        ...
        /*Copy the current data to the new image data, using the existing pixel for 
        coordinates within the bounds of the old image size*/
        for(unsigned i=0;i<newWidth;i++){
            for(unsigned j=0;j<newHeight;j++){
                if(i<width_ && j<height_){
                    HSLAPixel & oldPixel=this->getPixel(i,j);
                    HSLAPixel & newPixel=newImageData[(i+(j*newWidth))];
                    newPixel=oldPixel;
                }
            }
        }
